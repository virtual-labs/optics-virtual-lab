...
var slit_pointer_1, slit_pointer_2, hit_flag;
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            function loadImages(image, name, xPos, yPos, cursor, sFactor, alpha, container) {
                ...
                }else if ( name == "vernier_table"  || name == "vernier_scale" || name == "main_scale" || name == "prism" || name == "prism_hanger" || name == "main_1_scale_zoom" || name == "vernier_1_scale_zoom" || name == "main_2_scale_zoom" || name == "vernier_2_scale_zoom" ) { // Set rotation center
...
function displaySlit(scope){
    ...
    if(scope.switch_on_off==false){
        for(var i=1;i<color_slit.length;i++){         
            zoom_container.getChildByName(color_slit[i]).alpha = scope.place_prism ? 0 : 1;
        } 
        zoom_container.getChildByName("white_slit").alpha = scope.place_prism ?  1 : 0;
    }else{
        for(var i=1;i<color_slit.length;i++){         
...
function reflectionCalculation(scope){
    ...
    for(var i=0;i<refractive_index.length;i++){
        /**when absolute value of ((vernier)+angle of prism/2-90) greater than 90 
        then set the incident angle as (vernier)+angle of prism/2-90)*/
        if(Math.abs(scope.vernier_model + scope.vernier_fine_model + _angle_of_prism/2 - 90)>90){
            _incident_angle_ab[i] = NaN; /** Angle set to null */
        }else{
            /**I(AB)= (vernier + angle of the prism/2-90)*/
            _incident_angle_ab[i] = scope.vernier_model + scope.vernier_fine_model + _angle_of_prism/2 - 90;      
        }
    }
    for(var i=0;i<refractive_index.length;i++){
        /**finding the angle of incidence Ac, check whether absolute value of  
       -(90+(vernier - angle of prism/2) is greater than 90)*/
        if(Math.abs(-(90 + (scope.vernier_model + scope.vernier_fine_model - _angle_of_prism/2))) > 90){
            _incident_angle_ac[i] = NaN;/** Angle set to null */
        }else{
            /**I(AC)= -(90+(vernier -angle of the prism/2)*/
            _incident_angle_ac[i] = -(90 + (scope.vernier_model + scope.vernier_fine_model - _angle_of_prism/2));
        }
    }
    for(var i=0;i<refractive_index.length;i++){
        /** Reflection from AB (D)=i(AC)+SIN-1(SIN(A)* √(n2/n2s-SIN2(i))-COS(A)*SIN(i))-A */       
        if(Math.abs(scope.vernier_model+scope.vernier_fine_model+_angle_of_prism/2-90)>90){
            ...
            if((180 - 2 *_incident_angle_ab[i]) > 180){
                reflection_ab[i] = (180 - 2 * _incident_angle_ab[i]) - 360; /** Reflection from AB = (180 - 2 * incident angle) - 360 */
            }else{
                reflection_ab[i] = 180 - 2 * _incident_angle_ab[i];/** Reflection from AB = 180 - 2 * incident angle */
            }          
        }
    }    
    for(var i=0;i<refractive_index.length;i++){
         /** Reflection  from AC D=i(AB)+SIN-1(SIN(A)* √(n2/n2s-SIN2(i))-COS(A)*SIN(i))-A */
        if(Math.abs(-(90 + (scope.vernier_model + scope.vernier_fine_model - _angle_of_prism/2))) > 90){
            ...
            if((180+2*_incident_angle_ac[i])>180){
                reflection_ac[i] = (180 + 2*_incident_angle_ac[i]) - 360;/** Reflection from AC = (180 + 2* incident angle) - 360 */
            }else{        
                reflection_ac[i] = 180 + 2*_incident_angle_ac[i];  /** Reflection from AC = 180 + 2* incident angle */  
            }
        }
    }
}
...
function slitMovement(scope){  
    stageUpdate(); /** Apply stage update */
    /** Apply center slit mask movement  using tween w r to telescope and vernier rotation*/
    leftDirectionTween(slit_center_mask,1+(parseFloat(scope.telescope_model)-parseFloat(scope.vernier_model))*4);
    /** Apply white slit movement  using tween w r to telescope  rotation*/
    leftDirectionTween(zoom_container.getChildByName("white_slit"),slit_center_x+parseFloat(scope.telescope_model)*4);
    /** Apply right side slits movements using tween w r to telescope and reflection from ac*/
    ...
    /** Apply left side slits movements using tween w r to telescope and reflection from ab*/
        ...
        if(scope.place_prism==false){ /** Place prism */
            if(parseFloat(scope.vernier_model) >= -150 && parseFloat(scope.vernier_model) <= -88){
               ...
               for(var i=1;i<color_slit_left.length;i++){         
                ...
                /** Display left side white color slit    */
            ...
            if(parseFloat(scope.vernier_model) >= 88 && parseFloat(scope.vernier_model) <= 150){
                /** Display right side color slits    */
               for(var i=1;i<color_slit_right.length;i++){ 
                 ...
                 /** Display right side white color slit    */
