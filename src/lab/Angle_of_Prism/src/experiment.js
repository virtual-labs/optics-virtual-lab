...
var scale_mask, slit_pointer_1, slit_pointer_2, hit_flag;
...
var telescope_fine_angle, vernier_fine_angle, help_array;
    ...
    return {
        ...
        link: function(scope, element, attrs) {
            ...
            function loadImages(image, name, xPos, yPos, cursor, sFactor, alpha, container) {
                ...
                else if (name == "slit_center" || name == "slit_left" || name == "slit_right"  ) { /** Mask and blur apply on the slit */
...
function reflectionCalculation(scope){
   ...
   /** To convert into degree for trignometric calculation */
    ...
    if(Math.abs(-(90 + (scope.vernier_model + scope.vernier_fine_model - _angle_of_prism/2))) > 90){
		...
		/**(180+2*angle of incidence)-360,)*/
            reflection_ac = (180 + 2*_incident_angle_ac) - 360;            
        }else{
		/**reverse rotation (180+2*angle of incidence)*/
...
function slitMovement(scope){  
    ...
    /** Apply center slit movement  using tween w r to telescope  rotation*/
    leftDirectionTween(zoom_container.getChildByName("slit_center"),slit_center_x+(parseFloat(scope.telescope_model))*10);
     /** Apply right side slit movements using tween w r to telescope and reflection from ac*/
    ...
    /** Apply left side slit movements using tween w r to telescope and reflection from ab*/
